...
| `TracerProvider`      | `trace.TracerProvider`             | `nil`            | OTel `TracerProvider` for internal operation spans (writes, reads, WAL, quorum waits). `nil` falls back to `otel.GetTracerProvider()` — a no-op unless the embedding application has configured a global provider. Pass `trace.NewNoopTracerProvider()` to explicitly disable tracing.                                                                                                                                                                                                                                                                                                                                                                                      |
